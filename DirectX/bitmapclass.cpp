...
bool BitmapClass::Render(ID3D11DeviceContext* deviceContext)
{
	...
	result = UpdateImgSize(deviceContext);
	if (!result)
	{
		return false;
	}
...
bool BitmapClass::UpdateImgSize(ID3D11DeviceContext* deviceContent)
{
	float left, right, top, bottom;
	VertexType* vertices;
	D3D11_MAPPED_SUBRESOURCE mappedResource;
	...
	// If the position we are rendering this bitmap to hasn't changed then don't update the vertex buffer.
	if ((m_prevWidth == m_bitmapWidth) && (m_prevHeight == m_bitmapHeight))
	{
		return true;
	}
...
void BitmapClass::SetRenderSize(int x, int y)
{
	m_bitmapWidth = x;
	m_bitmapHeight = y;
	return;
